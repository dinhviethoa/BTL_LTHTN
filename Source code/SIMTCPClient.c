...
#include "SIMTCPClient.h"		/* Include TCP Client header file */
...
void SIM_send_SMS(char* Phone_number, char *warn_mess)
{
	USART_SendString("AT+CMGF=1\r\n");
	WaitForExpectedResponse("OK");
	USART_SendString("AT+CMGS=");
	USART_SendString(Phone_number);
	USART_SendString("\r\n");
	WaitForExpectedResponse(">");
	USART_SendString(warn_mess);
	USART_TxChar(0x1A);
	CRLF_COUNT = 2;
	WaitForExpectedResponse("OK"); 
}
